...
class BillRepository {
    ...
    suspend fun updateBill(
        ...
        return try {
            ...
            val response = supabaseApi.updateBill(
                url = url,
                apiKey = supabaseKey,
                auth = "Bearer $token",
                idCondition = "eq.$billId",
                body = fields
            )
            if (response.isSuccessful) Result.success(Unit)
            else Result.failure(Exception("Update failed: HTTP ${response.code()}"))
        } catch (e: Exception) {
            Result.failure(e)
        }
    }
        ...
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                val billMeta = com.example.myapplication.data.model.BillMeta(
                    ...
                    llm_error = row["error_message"] as? String
                )
